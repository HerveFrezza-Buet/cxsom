...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \STATE \COMMENT{Every update stability is confrmed. We can set all the results as $\cxReady$}
    \FORALL{$\cxUpstatN$}
    \STATE $\cxUpstatO$
    \ENDFOR
...
This is what \ConceptDef{update patterns} (UP) are introduced for. Let us denote a \ConceptDef{relative data instance} (RDI) as $\cxDftRDI$, where $\cxShiftSet$ is a relative time shift. A RDI can be \ConceptDef{anchored} as a DI at time $\cxDftInst$. This consists in transforming $\cxDftRDI$ into $\cxAnchor$.
