...
class MapView(cx.tkviewer.At):
    def __init__(self, master, map_name, figsize=(8, 5), dpi=100):
        ...
        self.We_path  = cx.variable.path_from(root_dir, 'wgt', map_name+'/We')
        self.Wc_path  = cx.variable.path_from(root_dir, 'wgt', map_name+'/Wc')
...
class HistoView(cx.tkviewer.Refresh):
    def __init__(self, master, map_name, figsize=(8, 4), dpi=100):
    ...
    def on_draw(self):
        self.fig.clear()
        ax = self.fig.gca()
        values = np.array([v for (_, v) in cx.variable.data_range_full(cx.variable.path_from(root_dir, 'rlx', self.map_name+'/Cvg'))])
...
slider = cx.tkviewer.HistoryFromVariableSlider(root,
                                               'time instants',
                                               cx.variable.path_from(root_dir, 'wgt', 'X/We'))
...
hbox2 = tk.Frame(root)
h1    = HistoView(hbox2, 'X')
h2    = HistoView(hbox2, 'Y')
...
h1.widget().pack    (side=tk.LEFT, fill=tk.BOTH)
h2.widget().pack    (side=tk.LEFT, fill=tk.BOTH)
